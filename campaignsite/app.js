...
async function handleShareVoterView(shareToken) {
    ...
    async function doVerify() {
        ...
        if (!window.db) {
            if (errorEl) { errorEl.textContent = 'Unable to connect. Please open the share link in the same site (e.g. paste the full URL in the browser).'; errorEl.style.display = 'block'; }
            return;
        }
        if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Verifying…'; }
        try {
            const { doc, getDoc } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
            const snap = await getDoc(doc(window.db, 'sharedVoterSnapshots', (shareToken || '').trim()));
            if (!snap.exists()) {
...
window.loadShareVoterList = async function () {
    ...
    try {
        ...
        const voters = (data.voters || []).map(v => {
            ...
            if (pu != null && typeof pu === 'object' && pu.pledge != null) {
                pledge = (pu.pledge || 'undecided').toString().toLowerCase().replace('negative', 'no');
                pledgeCandidateIds = Array.isArray(pu.candidateIds) ? pu.candidateIds : [];
            } else if (pu != null) {
                pledge = (pu || v.pledge || 'undecided').toString().toLowerCase().replace('negative', 'no');
            } else {
